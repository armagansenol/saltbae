  ...
  ${Object.entries(value)
    ...
    .map(([key, value]) => {
      if (key === 'font-size') {
        ...
        const responsiveValue = value as { mobile?: number; desktop?: number }
        return [
          `font-size: ${scalingCalc(responsiveValue.mobile!)};`,
          `@variant dt { font-size: ${scalingCalc(
...
@utility font-self-written {
  font-family: var(--font-self-written);
}
